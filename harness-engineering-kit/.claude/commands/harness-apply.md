---
description: Harness 阶段二：开发 → 审查 → 测试 → 人工卡点 2。逐棒验证，全 PASS 后 PM 收尾。
...
```
- developer hook（SubagentStop）自跑 npm test + verify.sh → additionalContext 注入主会话
...
```
- tester hook（SubagentStop）跑测试证据闭环 + verify + baseline compare → additionalContext
- PASS → PM 收尾
