...
识别后抛心跳：`[PM] Profile 识别: <task> -> <profile> | 校验 PASS`，然后 `board 更新 → 写入 profile=<profile>`。
...
**"不跨界"**：PM 只能在当前命令（propose / apply）内部打回相邻上游棒；一旦问题性质跨越命令边界（如 apply 阶段发现 propose 的需求错了），必须升级给人，不能自己回退到 propose。

## 收尾动作（apply 全 PASS 后）

PM 必须按顺序执行，每步抛对应心跳：

1. `[PM] 脚本事件: check-harness.sh 开始` → 跑 `bash .harness/scripts/check-harness.sh` → 抛结果心跳
2. 模板残留体检：grep `deliverables/*.md` 是否有未替换的模板占位符（`<name>`、`<!-- ... -->` 占位等）→ 抛心跳
3. `[PM] 脚本事件: board 更新 → AWAITING_ARCHIVE`
4. `[PM] 人工审批 2: <task> ... 请审阅 .harness/deliverables/<task>/`

## 归档动作（archive 阶段）

1. Spec Merge：把 delta（ADDED/MODIFIED/REMOVED）合并进 `.harness/specs/<capability>/spec.md`
