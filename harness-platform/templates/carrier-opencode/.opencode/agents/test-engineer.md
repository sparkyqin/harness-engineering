...
# Test Engineer(TE)— OpenCode 子代理入口

> 完整契约见 `.harness/agents/test-engineer.md`。
> 你是 apply 链路最后一棒。你停止后 OpenCode plugin(task 工具 + subagent_type=test-engineer)会跑测试证据闭环 + verify + baseline。
...
- Dev 改动的代码
- `.harness/agents/test-engineer.md`(完整契约)
