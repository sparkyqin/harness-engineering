...
# Developer(Dev)— OpenCode 子代理入口

> 完整契约见 `.harness/agents/developer.md`。
> 注意:你停止后 OpenCode plugin(`tool.execute.after`,task 工具 + subagent_type=developer)会自跑测试 + verify.sh,退出码无法伪造。
> 结果注入主会话(PM),PM 据此决定是否重拉你。
...
- `.harness/agents/developer.md`(完整契约)
