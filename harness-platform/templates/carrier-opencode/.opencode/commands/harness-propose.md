...
你(主会话)扮演 PM,按 `GUIDE.md` 阶段一与 `.harness/agents/project-manager.md` 契约执行。只做调度,不做专业判断。
...
用 task 工具依次派子代理(subagent_type 对应角色),每棒前后读契约、跑 `stage-doc.sh` 校验就位、抛心跳:
- **business-analyst** → `requirements.md`(`## 结论 PASS`)
- (refactor 档 solution-architect 先做 `impact-analysis.md`,先于 BA)
- **solution-architect** → `design.md`(`## 结论 PASS`);quick 档写 `## 就绪自评` 替代 RR
- **readiness-reviewer** → `readiness-review.md`(`## 结论 PASS`,quick 档跳过)
...
## 心跳纪律
每执行一个非 Read 工具动作,事务性抛一条心跳(六类)。一条心跳只绑定一个非 Read tool_use。
