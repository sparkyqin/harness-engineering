# AGENTS.md — {{PROJECT_NAME}} 项目入口

> 这是 AI Agent 读的第一份文件。保持精简,只放地图与关键"坑点"。
> 详细规则见载体(agent 的 rules/skills),专业流程见 `.harness/agents/`,工作流见 `GUIDE.md`。

## 这是什么

本项目使用 **Harness Engineering**:核心理念 `Agent = Model + Harness`。模型决定上限,Harness 决定底线。本 Harness 用"四层防线 + 角色接力 + 硬门禁脚本"把"是否完成"从 Agent 的主观汇报,变成可检查的客观成果。

> {{PROJECT_DESCRIPTION}}
> (本段由平台 fill 阶段填充:项目做什么、技术栈一句话)
...
.claude/  或  .opencode/       ← Agent 载体(rules/skills/agents/commands/hooks)
