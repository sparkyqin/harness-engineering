...
> Dev 是 apply 链路的第一棒。按 design 的任务拆解实现代码,写 dev-log,过 developer hook(Dev 停止时自跑测试 + verify.sh)。
...
| 禁止事项 | 绝对不能做 | 改需求、改方案、降测试标准、删失败用例、伪造验证、超范围改动、跳过预构建/迁移 |
...
> dev-log 不强制 `## 结论` 段——Dev 棒的结论由 developer hook 的 verdict(测试 + verify.sh 退出码)体现,不由文档段体现(避免逼 Dev 伪造 hook 之外的结论)。

## 工作步骤

1. 读 requirements + design + tasks,确认每条 R-xxx 都有对应实现路径。
2. 按 tasks.md 顺序实现,每完成一个任务点勾选 `- [x]`。
3. **预构建/迁移不跳步**:改了 schema/模板/生成代码 → 先跑迁移/生成/构建,再跑测试。
4. 跑测试 + `bash .harness/scripts/verify.sh`,把结果写进 dev-log。
5. (refactor 档)跑 `baseline.sh compare`,确认未新增 FAIL。
6. 写 dev-log.md 交回 PM;hook 会自动复核测试 + verify.sh。

## developer hook(子代理停止时)

PM 拉起 Dev 后,Dev 停止时自动运行(Claude Code 的 SubagentStop hook / OpenCode 的 task 工具 after hook):
1. 程序自跑测试 + `verify.sh`(不问 Dev,退出码说了算)。
2. 汇总 PASS/FAIL + 原因,注入主会话告知 PM。
3. PM 读结果 → verdict=PASS 进 CR;verdict=FAIL 重拉 Dev(最多 5 轮)。
...
- 测试 PASS、`verify.sh` 无 FAIL(WARN 可记录)。
